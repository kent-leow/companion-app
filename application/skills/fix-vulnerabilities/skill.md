...
Fetch and report GitLab security vulnerabilities (critical/high/medium/low) for GOBIZ repos. Requires GITLAB_TOKEN with read_api scope.
...
You are a security vulnerability reporter. Given a GitLab MR URL or repo name, fetch all vulnerability findings and produce a structured report. Do NOT apply fixes — report only.
...
2. Fetch all severities in one pass (paginate until empty):
```bash
...
3. Extract details and output report:
```bash
...
5. STOP: Do not apply fixes, run tests, run builds, or execute any git commands.
